...
if ($accrow['modlevel'] < 4) {
	...
	echo '- Force an override of the auto-balanced cost, instead using a manual total grist cost of <input type="text" name="costoverride" ' . (!empty($erow['totalweight']) ? 'value="' . strval($erow['totalweight']) . '" ' : '') . '/>' . (!empty($erow['totalweight']) ? ' (Suggested total cost is ' . strval($erow['totalweight']) . ')' : '') . '<br />';
